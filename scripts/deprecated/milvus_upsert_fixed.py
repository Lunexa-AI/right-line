...
Usage:
    python scripts/milvus_upsert_fixed.py [--input_file PATH] [--batch_size INT] [--verbose]
...
def upload_to_milvus(collection: Collection, data: List[Dict[str, Any]], batch_size: int = 100, verbose: bool = False) -> None:
    ...
    for i in tqdm(range(0, len(data), batch_size), desc="Uploading to Milvus", unit="batch"):
        ...
        for item in batch:
            # Ensure doc_type is truncated to max length (20)
            ...
            languages.append(item.get("language", "eng")[:10])  # Max length 10
            courts.append(item.get("court", "unknown")[:100])   # Max length 100
            date_contexts.append(item.get("date_context", "unknown")[:32])  # Max length 32
